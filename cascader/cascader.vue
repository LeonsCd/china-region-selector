...
<script lang="ts">
  ...
  import Trigger, { TriggerProps } from '@arco-design/web-vue/es/trigger'
  import SelectView from '@arco-design/web-vue/es/_components/select-view/select-view'
  import BaseCascaderPanel from '@arco-design/web-vue/es/cascader/base-cascader-panel'
  import CascaderSearchPanel from '@arco-design/web-vue/es/cascader/cascader-search-panel'
  import {
    CascaderFieldNames,
    CascaderOption,
    CascaderOptionInfo,
  } from '@arco-design/web-vue/es/cascader/interface'
  import {
    isArray,
    isFunction,
    isNull,
  ...
  import {
    ...
    Data,
    UnionType,
  } from '@arco-design/web-vue/es/_utils/types'
  import { useSelectedPath } from '@arco-design/web-vue/es/cascader/hooks/use-selected-path'
  import {
    KEYBOARD_KEY,
    getKeyDownHandler,
  } from '@arco-design/web-vue/es/_utils/keyboard'
  ...
  export default defineComponent({
    ...
    setup(props, { emit, slots }) {
      ...
      const newValue = ref(props.defaultValue)
      const newInputValue = ref(props.defaultInputValue)
      const newPopupVisible = ref(props.defaultPopupVisible)

      const { mergedDisabled, eventHandlers } = useFormItem({ disabled })

      watch(modelValue, (value) => {
        if (isUndefined(value) || isNull(value)) {
          newValue.value = props.multiple ? [] : undefined
        }
      })
      ...
      const computedValueMap = computed(() => {
        const values = getValidValues(props.modelValue ?? newValue.value, {
      ...
      const computedInputValue = computed(
        () => props.inputValue ?? newInputValue.value
      )
      const computedPopupVisible = computed(
        () => props.popupVisible ?? newPopupVisible.value
      )
              ...
              if (computedPopupVisible.value) {
                if (activeOption.value) {
                  ...
                  if (props.checkStrictly || activeOption.value.isLeaf) {
                    checked = !computedValueMap.value.has(
